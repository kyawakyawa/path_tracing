...
int main(int argc, char **argv){
	...
	/*const toml::Value* ct = v.find("camera.transform");

	for(const auto &v : ct->as<toml::Array>()){
		Scene_data::Transform transform(v);
		std::cout << transform.type << std::endl;
		for(int i = 0;i < 3;i++){
			std::cout << transform.vec3[i] << std::endl;
		}
	...
	std::cout << "\n\n" << std::endl;*/
	/*const toml::Value* me = v.find("mesh");

	for(const auto &v : me ->as<toml::Array>()){
		Scene_data::Mesh mesh(v);
	...
	std::cout << "\n\n" << std::endl;
	const toml::Value* ma = v.find("material");

	for(const auto &v : ma ->as<toml::Array>()){
		Scene_data::Material material(v);
		std::cout << material.name<< std::endl;
		std::cout << material.type << std::endl;
		std::cout << material.albedo<< std::endl;
		std::cout << material.emission << std::endl;
	}
	//delete ma;

	std::cout << "\n\n" << std::endl;
	const toml::Value* ca = v.find("camera");

	Scene_data::Camera camera(*ca);
	...
	std::cout << "\n\n" << std::endl;
	const toml::Value* ob = v.find("object");

	for(const auto &v : ob->as<toml::Array>()) {
		Scene_data::Object object(v);
		std::cout << object.name << std::endl;
		std::cout << object.mesh << std::endl;
		std::cout << object.material << std::endl;
		for(const auto &transform : object.transform){
			std::cout << transform.type << std::endl;
			for(int i = 0;i < 3;i++){
				std::cout << transform.vec3[i] << std::endl;
			}
	...
	std::cout << "\n\n" << std::endl;
	const toml::Value* li = v.find("light");

	for(const auto &v : li->as<toml::Array>()) {
		Scene_data::Light light(v);
		std::cout << light.emission << std::endl;
	...
	std::cout << "\n\n" << std::endl;
	const toml::Value* sk = v.find("sky");

	Scene_data::Sky sky(*sk);

	std::cout << sky.type << std::endl;
	std::cout << sky.color << std::endl;
	std::cout << sky.path << std::endl;
	std::cout << sky.theta_offset << std::endl;
	std::cout << sky.phi_offset << std::endl;
	//delete sk;

	std::cout << "\n\n" << std::endl;
	const toml::Value* fi = v.find("film");

	Scene_data::Film film(*fi);

	std::cout << film.resolution[0] << " " << film.resolution[1] << std::endl;
	std::cout << film.output << std::endl;
	std::cout << film.gamma << std::endl;
	std::cout << film.sensitivity << std::endl;
	//delete fi;

	std::cout << "\n\n" << std::endl;
	const toml::Value* re = v.find("renderer");

	Scene_data::Renderer renderer(*re);
	...
	std::cout << renderer.no_direct_emitter << std::endl;
	std::cout << renderer.threads << std::endl;
	std::cout << renderer.integrator << std::endl;*/
	//delete re;
