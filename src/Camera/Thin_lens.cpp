...
Thin_lens::Thin_lens(const int w,const int h,const Vec3 &p,const Vec3 &d,const Vec3 &u,const R fov_,const R focus_distance/*レンズからの焦点の合う距離(焦点距離ではない、焦点距離はこの値の半分)*/,const R f_number_/*F値*/) : Camera(w,h,p,d,u)
...
void Thin_lens::sensitization(std::vector<Shape*> *shapes,std::vector<Light_source*> *lights,const int samples,const int threads) {
        if(radiance == nullptr) {
        ...
        for(int i = 0;i < pixel_h;i++) {
            for(int j = 0;j < pixel_w;j++) {
                for(int k = 0;k < samples;k++) {
                    ...
                    const R W = 0.0005 * f_number * f_number *  pixel_h * pixel_w / height_of_sensor * width_of_sensor;
