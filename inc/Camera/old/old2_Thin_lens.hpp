...
struct Thin_lens : Camera {
    ...
    const R f_number;
    const R lens_radius;
    const R focus_distance;

    const R dist_lens_object_plane;
    const R width_of_object_plane;
    const R height_of_object_plane;
    const Vec3 object_plane_center;

    const R pdf_i;
    const R pdf_l;

    Thin_lens() = delete;
    inline Thin_lens(const int w,const int h,const Vec3 &p,const Vec3 &d,const Vec3 &u,const R fov_,const R focus_distance_,const R f_number_) : Camera(w,h,p,d,u)
        ,x(cross(dir,up).normalized()),y(cross(dir,x).normalized())
        ,dist_sensor_lens(2.0 * focus_distance_)
        ...
        ,f_number(f_number_),lens_radius(focus_distance_ / f_number_ / 2.0),focus_distance(focus_distance_)

        ,dist_lens_object_plane(focus_distance * dist_sensor_lens / (dist_sensor_lens - focus_distance))
        ,width_of_object_plane(width_of_sensor * dist_lens_object_plane / dist_sensor_lens)
        ,height_of_object_plane(height_of_sensor * dist_lens_object_plane / dist_sensor_lens)
        ,object_plane_center(dir * dist_lens_object_plane)
