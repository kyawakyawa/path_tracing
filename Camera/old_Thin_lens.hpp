...
struct Thin_lens : Camera {
    ...
    const R width_of_object_plane= 1.0;
    const R height_of_object_plane = 1.0;
    const Vec3 object_center;

    const Vec3 lens_center = Vec3(0,0,1);
    const R lens_radius = 1.0;

    const R pdf_i = 1.0;
    const R pdf_l = 1.0 / M_PI;

    Thin_lens() = delete;
    inline Thin_lens(const int w,const int h,const Vec3 &p,const Vec3 &d,const Vec3 &u,const R dist,const R f) : Camera(w,h,p,d,u)
        ,width_of_sensor(1.0),height_of_sensor(1.0 * h / w),A(dist),F(f),B(1.0 /(1.0 / f - 1.0 / A))
